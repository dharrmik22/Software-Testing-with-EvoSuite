...
import edu.ksu.cis.projects.bogor.kiasan.region.spec.Region;
 ...
 * new collection implementations, Vector is synchronized.
 * <p>
 * 
...
public class Vector<E> extends java.util.AbstractList<E> implements List<E>,
	 ...
	 * at index <tt>k</tt> in this vector is copied into component <tt>k</tt> of
	 ...
	 */
	@Assertion(@Case(pre = "repOK()", post = "repOK() && minCapacity<=elementData.length"))
	 ...
	 * @see Object#equals(Object)
	 */
	@Assertion(@Case(pre = "Region.arrayReg(this.elementData,0,1) && Region.setObjReg(elem,1) && repOK() && index >=0", post = "repOK()&&($ret==-1 || ($ret>=index && elementData[$ret]==elem))"))
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if <tt>index</tt> is greater than or equal to the current
	 *                size of this vector.
	 */
	@Assertion(@Case(pre = "Region.arrayReg(this.elementData,0,1) && Region.setObjReg(elem,1) && repOK() ", post = "repOK() &&($ret==-1 || ($ret<=index && elementData[$ret]==elem))"))
	 ...
	 *                current size of this <tt>Vector</tt> object. given.
	 * @see #get(int)
	 * @see List
	 */
	 ...
	 * @see List
	 */
	@Assertion(@Case(pre = "repOK() ", post = "repOK()"))
	 ...
	 * @see List
	 */
	@Assertion(@Case(pre = "repOK() && index >=0 && index<=elementCount", post = "repOK()"))
	...
	boolean repOK() {
		return elementData != null && 0 <= elementCount
	...
	/**
	 * Save the state of the <tt>Vector</tt> instance to a stream (that is,
	...
	@Auto
	public int spex_M_indexOf(Object elem, int index) {
		boolean b0 = false;
		try {
			b0 = Region.arrayReg(this.elementData, 0, 1)
					&& Region.setObjReg(elem, 1) && repOK() && index >= 0;
			pre(b0);
		} catch (Throwable $t) {
			pre(false);
		}
		{
			int $ret = spex_M_indexOf(elem, index);
			if (b0)
				post(repOK()
						&& ($ret == -1 || ($ret >= index && elementData[$ret] == elem)));
			return $ret;
		}
	}

	@Auto
	public int spex_M_lastIndexOf(Object elem, int index) {
		boolean b0 = false;
		try {
			b0 = Region.arrayReg(this.elementData, 0, 1)
					&& Region.setObjReg(elem, 1) && repOK();
			pre(b0);
		} catch (Throwable $t) {
			pre(false);
		}
		{
			int $ret = spex_M_lastIndexOf(elem, index);
			if (b0)
				post(repOK()
						&& ($ret == -1 || ($ret <= index && elementData[$ret] == elem)));
			return $ret;
	...
	@Auto
	public void spex_M_ensureCapacity(int minCapacity) {
		boolean b0 = false;
		try {
			b0 = repOK();
			pre(b0);
		...
		{
			spex_M_ensureCapacity(minCapacity);
			if (b0)
				post(repOK() && minCapacity <= elementData.length);
			return;
		}
	}

	@Auto
	public void spex_M_insertElementAt(E obj, int index) {
		boolean b0 = false;
		try {
			b0 = repOK() && index >= 0 && index <= elementCount;
			pre(b0);
		} catch (Throwable $t) {
			pre(false);
		}
		{
			spex_M_insertElementAt(obj, index);
			if (b0)
				post(repOK());
			return;
		}
	...
	@Auto
	public void spex_M_removeElementAt(int index) {
		boolean b0 = false;
		try {
			b0 = repOK();
			pre(b0);
